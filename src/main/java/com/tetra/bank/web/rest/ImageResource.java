...
import com.tetra.bank.service.ImageService;
...
/**
 * REST controller for managing {@link com.tetra.bank.domain.Image}.
 */
@RestController
@RequestMapping("/api")
public class ImageResource {
     ...
     */
    @GetMapping("/images")
    public List<ImageDTO> getAllImages(@RequestParam(required = false) String filter) {
        if ("client-is-null".equals(filter)) {
            log.debug("REST request to get all Images where client is null");
            return imageService.findAllWhereClientIsNull();
        }
